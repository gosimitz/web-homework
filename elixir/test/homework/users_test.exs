defmodule Homework.UsersTest do
  ...
  describe "users" do
    ...
    setup do
      {:ok, company1} = Companies.create_company(%{credit_line: 5280, name: "some company"})
      {:ok, company2} = Companies.create_company(%{credit_line: 271828, name: "some company"})
    ...
    test "create_user/1 with valid data creates a user", %{
      ...
      assert user.company_id == valid_attrs.company_id
    end
    ...
    test "delete_user/1 on a nonexistent user", %{valid_attrs: valid_attrs} do
      user = user_fixture(valid_attrs)
